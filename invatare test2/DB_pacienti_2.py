...
def creare_baza_date():
    ...
    for i in range(n):
        pacient = {
            'ID': input(f'ID Pacient {i + 1}: '),
            'Nume': input(f'Nume Pacient {i + 1}: '),
            'Prenume': input(f'Prenume Pacient {i + 1}: '),
            'TS': float(input(f'Tensiune Sistolica (TS) Pacient {i + 1}: ')),
            'TD': float(input(f'Tensiune Diastolica (TD) Pacient {i + 1}: '))
        }
...
def afisare_lista_pacienti(baza_date):
    ...
    for pacient in baza_date:
        print(f"{pacient['ID']} \t {pacient['Nume']} \t {pacient['Prenume']} \t {pacient['TS']} \t {pacient['TD']} \t {pacient['TM']}")
...
def fisier_iesire(baza_date):
    with open('baza_date_pacienti.txt', 'w') as file:
        ...
        for pacient in baza_date:
            file.write(f"{pacient['ID']} \t {pacient['Nume']} \t {pacient['Prenume']} \t {pacient['TS']} \t {pacient['TD']} \t {pacient['TM']}\n")
